...
videos = sanity_query('*[_type == "video"]{ _id, title, duration }')
print(f"Total videos: {len(videos)}")

short = [v for v in videos if iso_to_secs(v.get("duration")) <= 120]
print(f"Videos under 2 min to delete: {len(short)}")
for v in short[:10]:
    print(f"  {v['_id']} | {v.get('duration','?')} | {v.get('title','?')[:60]}")
if len(short) > 10:
...
print(f"DONE. Deleted {deleted} short videos from Sanity.")
