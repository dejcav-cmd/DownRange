...
result = {}
result["screenshot_article"] = sanity_query(
    '*[_type=="newsArticle" && (summary match "*Goggins*" || excerpt match "*Goggins*" || body match "*Goggins*")][0]{_id, title, summary, excerpt}',
    SANITY_TOKEN
).get("result")
result["remaining_cdata_count"] = sanity_query(
    'count(*[_type=="newsArticle" && (summary match "*CDATA*" || excerpt match "*CDATA*" || body match "*CDATA*")])',
    SANITY_TOKEN
).get("result")

with open("verify_cdata_fix_result.json", "w") as f:
