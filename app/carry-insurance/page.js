  ...
  return (
    <>
      ...
      <div style={{ padding:'40px 0' }}>
        <div className="container">
          <div style={{ background:'#1A0000', border:'1px solid #7F1D1D', padding:'14px 20px', fontFamily:'monospace', fontSize:'12px', color:'#FCA5A5', marginBottom:'28px', lineHeight:1.7 }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:'16px', marginBottom:'40px' }}>
            {PLANS.map((p,i)=>(
              <div key={p.name} style={{ background:'#111318', border:`1px solid ${i===0?'#C8922A':'#1F2428'}`, padding:'20px', position:'relative' }}>
                {i===0&&<div style={{ position:'absolute', top:-10, left:'50%', transform:'translateX(-50%)', background:'#C8922A', color:'#000', fontFamily:'monospace', fontSize:'9px', fontWeight:700, padding:'3px 10px', whiteSpace:'nowrap' }}>EDITOR'S PICK</div>}
                <div style={{ fontFamily:"'Bebas Neue', sans-serif", fontSize:'1.6rem', color:'#C8922A', letterSpacing:'0.03em', marginBottom:'2px' }}>{p.name}</div>
                <div style={{ fontFamily:'monospace', fontSize:'9px', color:'#4B5563', marginBottom:'12px' }}>{p.tier} Tier</div>
                ...
                <div style={{ fontFamily:'monospace', fontSize:'10px', color:'#4B5563', marginBottom:'16px' }}>${p.annual}/year</div>
                <div style={{ display:'flex', flexDirection:'column', gap:'6px', marginBottom:'14px' }}>
                  {[['Civil',p.coverage],['Criminal',p.criminal],['Bail',p.bail]].map(([k,v])=>(
                    <div key={k} style={{ fontFamily:'monospace', fontSize:'10px', color:'#6B7280' }}>
                ...
                <p style={{ fontFamily:'monospace', fontSize:'10px', color:'#4B5563', lineHeight:1.6, marginBottom:'14px' }}>{p.verdict}</p>
                <a href={p.url} target="_blank" rel="noopener noreferrer"
                  style={{ display:'block', textAlign:'center', background:'transparent', border:'1px solid #C8922A', color:'#C8922A', fontFamily:'monospace', fontWeight:700, fontSize:'11px', padding:'8px', textDecoration:'none' }}>
                  VIEW PLANS →
                </a>
              </div>
            ))}
          ...
          <div style={{ fontFamily:'monospace', fontSize:'11px', color:'#374151', marginBottom:'40px', lineHeight:1.7, padding:'16px 20px', background:'#111318', border:'1px solid #1F2428' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(2,1fr)', gap:'12px' }}>
            {[
              ...
              <div key={t} style={{ background:'#111318', border:'1px solid #1F2428', borderLeft:'3px solid #C8922A', padding:'16px 20px' }}>
                <div style={{ fontFamily:'monospace', fontSize:'13px', fontWeight:700, color:'#F0EDE6', marginBottom:'8px' }}>{t}</div>
                <p style={{ fontFamily:'monospace', fontSize:'11px', color:'#4B5563', lineHeight:1.7 }}>{d}</p>
              </div>
            ))}
          </div>
        </div>
