...
export async function POST(req) {
  ...
  const QUERIES = [
    `*[_type=="newsArticle"    && (${force ? 'true' : 'imageUrl match "/img/*.svg" || !defined(imageUrl) || imageUrl == ""'})] | order(publishedAt desc) [0...${batch}] { _id, title, imageUrl, sourceUrl, category }`,
    `*[_type=="blogPost"       && (${force ? 'true' : 'imageUrl match "/img/*.svg" || !defined(imageUrl) || imageUrl == ""'})] | order(_createdAt desc) [0...20] { _id, title, imageUrl, category }`,
    `*[_type=="firearmRelease" && (${force ? 'true' : 'imageUrl match "/img/*.svg" || !defined(imageUrl) || imageUrl == ""'})] | order(_createdAt desc) [0...30] { _id, "title": brand+" "+model, imageUrl, sourceUrl, category }`,
    `*[_type=="review"         && (${force ? 'true' : 'imageUrl match "/img/*.svg" || !defined(imageUrl) || imageUrl == ""'})] | order(_createdAt desc) [0...20] { _id, "title": brand+" "+model, imageUrl, category }`,
    `*[_type=="canadaContent"  && (${force ? 'true' : 'imageUrl match "/img/*.svg" || !defined(imageUrl) || imageUrl == ""'})] | order(_createdAt desc) [0...20] { _id, title, imageUrl, category }`,
  ]
