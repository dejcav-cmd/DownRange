...
export async function GET(req) {
  ...
  try {
    ...
    await reportCronRun('queue_digest', { status: res.ok ? 'success' : 'failed', ms: Date.now() - t0, details: JSON.stringify(d).slice(0,100) }).catch(()=>{})
    return Response.json(d)
  } catch (err) {
    await reportCronRun('queue_digest', { status: 'failed', ms: Date.now() - t0, error: err.message }).catch(()=>{})
