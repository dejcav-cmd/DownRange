...
export async function GET(req) {
  ...
  const cfg = platformCfg['instagram'] || {}

  // Only run if this platform is explicitly enabled in the scheduler
  if (!cfg.enabled) {
    return Response.json({ ok: true, skipped: true, reason: 'instagram is not enabled in the scheduler' })
  }

  const count  = cfg.postsPerRun ?? 1
  const result = await runSocialAgent({ platform: 'instagram', count }).catch(e => ({ ok: false, error: e.message }))
