...
export async function GET(req) {
  ...
  const cfg = platformCfg['twitter'] || {}

  // Only run if this platform is explicitly enabled in the scheduler
  if (!cfg.enabled) {
    return Response.json({ ok: true, skipped: true, reason: 'twitter is not enabled in the scheduler' })
  }

  const count  = cfg.postsPerRun ?? 1
  const result = await runSocialAgent({ platform: 'twitter', count }).catch(e => ({ ok: false, error: e.message }))
