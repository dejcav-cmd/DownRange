...
export async function GET(req) {
  ...
  const cfg = platformCfg['threads'] || {}

  // Only run if this platform is explicitly enabled in the scheduler
  if (!cfg.enabled) {
    return Response.json({ ok: true, skipped: true, reason: 'threads is not enabled in the scheduler' })
  }

  const count  = cfg.postsPerRun ?? 1
  const result = await runSocialAgent({ platform: 'threads', count }).catch(e => ({ ok: false, error: e.message }))
...
export async function POST(req) {
  ...
  const result = await runSocialAgent({ platform: 'threads', count: body.count ?? 2, dryRun: body.dryRun ?? false }).catch(e => ({ ok: false, error: e.message }))
  return Response.json(result)
}
