...
const CANONICAL_PLANS = [
  {
    name: 'USCCA', tier: 'Elite', monthly: 47, annual: 497,
    coverage: '$2M civil', criminal: 'Attorney fees paid upfront', bail: '$100K',
    ...
    rating: 9.4, url: 'https://www.uscca.com',
  ...
  {
    name: 'CCW Safe', tier: 'Ultimate', monthly: 55, annual: 659,
    coverage: 'Unlimited civil', criminal: 'Unlimited attorney fees', bail: 'Unlimited',
    ...
    rating: 9.2, url: 'https://ccwsafe.com',
  ...
  },
  {
    ...
    coverage: '$1M civil', criminal: '$150K criminal defense', bail: '$25K',
    training: 'Not included',
    verdict: 'Best budget choice. Significantly cheaper than the top two while still offering meaningful coverage limits.',
    rating: 8.5, url: 'https://www.secondcalldefense.org',
  ...
  {
    name: 'US Law Shield', tier: 'Defender', monthly: 11, annual: 131,
    coverage: 'Unlimited civil', criminal: 'Unlimited attorney', bail: 'Not included',
    training: 'Not included',
    verdict: 'Ultra-budget entry point. Unlimited coverage at the lowest price point.',
    rating: 8.0, url: 'https://www.uslawshield.com',
...
async function tryFetchPrice(url) {
  try {
    ...
    const patterns = [/\$(\d+)\.?\d*\s*\/\s*mo/i, /\$(\d+)\.?\d*\s*per\s*month/i, /\$(\d+)\.?\d*\s*a\s*month/i]
...
export async function GET(req) {
  ...
  try {
    ...
    for (const plan of CANONICAL_PLANS) {
      ...
      if (!url) continue
      const scrapedPrice = await tryFetchPrice(url)
      const delta = scrapedPrice ? Math.abs(scrapedPrice - plan.monthly) / plan.monthly : 0
      results.push({
        plan: plan.name, canonical: plan.monthly, scraped: scrapedPrice,
    ...
    const details = updated > 0
      ? `${updated} plan(s) need price update: ${results.filter(r=>r.action==='NEEDS_UPDATE').map(r=>r.plan).join(', ')}`
      : `All ${CANONICAL_PLANS.length} plan prices current within 5% tolerance`

    await reportCronRun('carry-insurance', { status: 'success', ms, details }).catch(() => {})
    console.log(`[carry-insurance] ${now} — checked ${CANONICAL_PLANS.length} plans, ${updated} need update`)

    return NextResponse.json({ ok: true, timestamp: now, checked: CANONICAL_PLANS.length, needsUpdate: updated, results,
      message: updated > 0
        ? `${updated} plan(s) have pricing changes >5%. Update CANONICAL_PLANS in the cron file.`
        : 'All plan prices current within 5% tolerance.',
    })
  } catch (err) {
    await reportCronRun('carry-insurance', { status: 'failed', ms: Date.now() - t0, error: err.message }).catch(() => {})
