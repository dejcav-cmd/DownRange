  ...
  if (!authed) return (
    <div style={{ background:C.bg, minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center' }}>
      <div style={{ background:C.bg2, border:`1px solid ${C.border}`, padding:'40px', width:360 }}>
        ...
        <p style={{ fontFamily:'monospace', fontSize:'12px', color:C.muted, marginBottom:24 }}>Enter CRON_SECRET to access</p>
        <form onSubmit={login} style={{ display:'flex', flexDirection:'column', gap:12 }}>
          ...
          <button type="submit" style={{ background:C.gold, color:'#000', border:'none', padding:12, fontFamily:'monospace', fontWeight:700, fontSize:14, cursor:'pointer' }}>ACCESS →</button>
        </form>
      </div>
    </div>
  )
    ...
    <div style={{ background:C.bg, minHeight:'100vh', color:C.text }}>
      ...
      <div style={{ background:C.bg2, borderBottom:`1px solid ${C.border}`, padding:'14px 32px', display:'flex', alignItems:'center', justifyContent:'space-between', gap:16 }}>
        <div style={{ display:'flex', alignItems:'center', gap:16 }}>
          ...
          {siteOk && <span style={{ fontFamily:'monospace', fontSize:'11px', color:siteOk.ok?C.green:C.red }}>● SITE {siteOk.ok?'ONLINE':'DOWN'} {siteOk.ok?`(${siteOk.ms}ms)`:''}</span>}
        </div>
        <div style={{ display:'flex', gap:12, alignItems:'center' }}>
          <Link href="/" style={{ fontFamily:'monospace', fontSize:'11px', color:C.muted, textDecoration:'none' }}>← SITE</Link>
          <Link href="/studio" style={{ fontFamily:'monospace', fontSize:'11px', color:C.muted, textDecoration:'none' }}>SANITY STUDIO →</Link>
          <button onClick={logout} style={{ background:'none', border:`1px solid ${C.border}`, color:C.muted, padding:'5px 12px', fontFamily:'monospace', fontSize:'11px', cursor:'pointer' }}>LOGOUT</button>
        </div>
      </div>
      ...
      <div style={{ background:C.bg2, borderBottom:`1px solid ${C.border}`, padding:'0 32px', display:'flex', gap:0, overflowX:'auto' }}>
        {TABS.map(t=>(
          <button key={t.key} onClick={()=>setTab(t.key)}
            style={{ background:'none', border:'none', borderBottom:`2px solid ${tab===t.key?C.gold:'transparent'}`, color:tab===t.key?C.gold:C.muted, padding:'12px 16px', fontFamily:'monospace', fontSize:'11px', cursor:'pointer', whiteSpace:'nowrap', letterSpacing:'0.03em' }}>
            {t.label}
          </button>
        ))}
      </div>
      ...
      <div style={{ padding:'28px 32px', maxWidth:1100 }}>
        ...
        {tab==='feeds' && (
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:32 }}>
            <div>
              <div style={{ fontFamily:'monospace', fontSize:'10px', color:C.gold, letterSpacing:'0.15em', marginBottom:14 }}>FEED CONTROLS</div>
              {FEEDS.map(f=>(
                <div key={f.key} style={{ background:C.bg2, border:`1px solid ${C.border}`, padding:'12px 16px', marginBottom:8, display:'flex', alignItems:'center', justifyContent:'space-between', gap:12 }}>
                  <div style={{ flex:1 }}>
                    <div style={{ fontFamily:'monospace', fontSize:'12px', color:C.text, fontWeight:600 }}>{f.icon} {f.label}</div>
                    <div style={{ fontFamily:'monospace', fontSize:'10px', color:C.muted, marginTop:2 }}>{f.schedule}</div>
                    {feedResults[f.key] && <div style={{ fontFamily:'monospace', fontSize:'10px', marginTop:4, color:feedResults[f.key].error?C.red:C.green }}>
              ...
              <div style={{ background:C.bg2, border:`1px solid ${C.border}`, padding:'12px 16px', display:'flex', alignItems:'center', justifyContent:'space-between', gap:12 }}>
                <div style={{ fontFamily:'monospace', fontSize:'12px', color:C.text, fontWeight:600 }}>🔍 Algolia Reindex</div>
                <Btn label="REINDEX" onClick={()=>runFeed('algolia')} loading={loadingFeed==='algolia'} variant='outline' small />
              </div>
            </div>
            <div>
              <div style={{ fontFamily:'monospace', fontSize:'10px', color:C.gold, letterSpacing:'0.15em', marginBottom:14 }}>QUICK LINKS</div>
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8 }}>
                {['/news','/laws','/deals','/market','/ranges','/guns','/reviews','/nfa-tracker','/compare/glock-19-vs-sig-p320','/training','/ffl-finder','/studio'].map(l=>(
                  <Link key={l} href={l} style={{ background:C.bg2, border:`1px solid ${C.border}`, padding:'8px 12px', fontFamily:'monospace', fontSize:'10px', color:C.muted, textDecoration:'none', display:'block' }}>
        ...
        {tab==='news' && (
          <div>
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:16 }}>
              <div style={{ fontFamily:'monospace', fontSize:'10px', color:C.gold, letterSpacing:'0.15em' }}>NEWS CATEGORIZATION</div>
            ...
            <p style={{ fontFamily:'monospace', fontSize:'11px', color:C.muted, marginBottom:20, lineHeight:1.7 }}>View and recategorize the last 50 published articles. Changes save to Sanity immediately.</p>
            {newsArticles.length===0&&!loadingNews&&<div style={{ textAlign:'center', padding:'40px', color:C.muted, fontFamily:'monospace', fontSize:'12px' }}>Click "Load Articles" to start.</div>}
                ...
                return (
                  <div key={a._id} style={{ background:C.bg2, border:`1px solid ${C.border}`, padding:'10px 14px', display:'grid', gridTemplateColumns:'1fr 180px 60px', gap:10, alignItems:'center' }}>
                    <div>
                      ...
                      <div style={{ display:'flex', gap:8, fontFamily:'monospace', fontSize:'10px', color:C.muted }}>
                        <span>{a.source}</span>
                    ...
                    <select value={a.category||'news'} onChange={e=>updateCategory(a._id,e.target.value)} disabled={catUpdating===a._id}
                      style={{ background:C.bg, border:`1px solid ${catColor}60`, color:catColor, padding:'5px 8px', fontFamily:'monospace', fontSize:'11px', cursor:'pointer' }}>
        ...
        {tab==='breaking' && (
          <div style={{ maxWidth:700 }}>
            <div style={{ fontFamily:'monospace', fontSize:'10px', color:C.gold, letterSpacing:'0.15em', marginBottom:16 }}>BREAKING ALERT MANAGER</div>
            <p style={{ fontFamily:'monospace', fontSize:'11px', color:C.muted, marginBottom:20, lineHeight:1.7 }}>Create breaking alerts that appear in the red ticker at the top of every page. These are automatically created when the news agent scores an article 8+/10.</p>
            <div style={{ background:C.bg2, border:`1px solid ${C.border}`, padding:'20px', marginBottom:20 }}>
              <div style={{ fontFamily:'monospace', fontSize:'10px', color:C.muted, marginBottom:12 }}>CREATE MANUAL ALERT</div>
              <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
                <input value={newAlert.headline} onChange={e=>setNewAlert(p=>({...p,headline:e.target.value}))} placeholder="Alert headline..."
                  style={{ background:C.bg, border:`1px solid ${C.border}`, color:C.text, padding:'10px', fontFamily:'monospace', fontSize:'12px' }} />
                <div style={{ display:'flex', gap:10 }}>
                  <div style={{ flex:1 }}>
                    <label style={{ fontFamily:'monospace', fontSize:'9px', color:C.muted, display:'block', marginBottom:4 }}>URGENCY (1-10)</label>
                    <input type="number" min="1" max="10" value={newAlert.urgencyScore} onChange={e=>setNewAlert(p=>({...p,urgencyScore:Number(e.target.value)}))}
                      style={{ width:'100%', background:C.bg, border:`1px solid ${C.border}`, color:C.red, padding:'8px 10px', fontFamily:'monospace', fontSize:'14px', fontWeight:700 }} />
                  </div>
                  <div style={{ flex:2 }}>
                    <label style={{ fontFamily:'monospace', fontSize:'9px', color:C.muted, display:'block', marginBottom:4 }}>LINK URL (optional)</label>
                    <input value={newAlert.url} onChange={e=>setNewAlert(p=>({...p,url:e.target.value}))} placeholder="https://..."
                      style={{ width:'100%', background:C.bg, border:`1px solid ${C.border}`, color:C.text, padding:'8px 10px', fontFamily:'monospace', fontSize:'12px' }} />
                  </div>
                </div>
            ...
            {alerts.length>0 && (
              <div>
                <div style={{ fontFamily:'monospace', fontSize:'10px', color:C.muted, marginBottom:10 }}>RECENT ALERTS</div>
                {alerts.map((a,i)=>(
                  <div key={a._id||i} style={{ background:C.bg2, border:`1px solid #7F1D1D`, padding:'10px 14px', marginBottom:6, display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                    <span style={{ fontFamily:'monospace', fontSize:'12px', color:C.red }}>● {a.headline}</span>
                    <span style={{ fontFamily:'monospace', fontSize:'10px', color:C.muted }}>{a.urgencyScore}/10</span>
                  </div>
                ))}
              </div>
            )}
        ...
        {tab==='channels' && (
          <div style={{ maxWidth:700 }}>
            <div style={{ fontFamily:'monospace', fontSize:'10px', color:C.gold, letterSpacing:'0.15em', marginBottom:14 }}>YOUTUBE CHANNELS</div>
            {channels.map(ch=>(
              <div key={ch.id} style={{ background:C.bg2, border:`1px solid ${ch.enabled?C.border:'#1A1A1A'}`, padding:'12px 14px', marginBottom:8, display:'flex', alignItems:'center', gap:12, opacity:ch.enabled?1:0.5 }}>
                ...
                <div style={{ flex:1 }}>
                  <div style={{ fontFamily:'monospace', fontSize:'12px', color:C.text, fontWeight:600 }}>{ch.name}</div>
                  <div style={{ fontFamily:'monospace', fontSize:'9px', color:C.muted }}>ID: {ch.id}</div>
                </div>
                <button onClick={()=>setChannels(p=>p.filter(c=>c.id!==ch.id))}
                  style={{ background:'none', border:'none', color:C.red, cursor:'pointer', fontFamily:'monospace', fontSize:'11px', padding:'3px 6px' }}>✕</button>
              </div>
            ))}
            <div style={{ background:C.bg2, border:`1px solid ${C.border}`, padding:'16px', marginTop:12, marginBottom:16 }}>
              <div style={{ fontFamily:'monospace', fontSize:'10px', color:C.muted, marginBottom:10 }}>ADD CHANNEL</div>
              <div style={{ display:'flex', gap:8 }}>
                <input value={newChId} onChange={e=>setNewChId(e.target.value)} placeholder="UCxxxxxxxxxx (Channel ID)"
                  style={{ flex:2, background:C.bg, border:`1px solid ${C.border}`, color:C.text, padding:'8px 10px', fontFamily:'monospace', fontSize:'11px' }} />
                <input value={newChName} onChange={e=>setNewChName(e.target.value)} placeholder="Name"
                  style={{ flex:1, background:C.bg, border:`1px solid ${C.border}`, color:C.text, padding:'8px 10px', fontFamily:'monospace', fontSize:'11px' }} />
                <button onClick={()=>{ if(!newChId||!newChName)return; setChannels(p=>[...p,{id:newChId.trim(),name:newChName.trim(),enabled:true}]); setNewChId(''); setNewChName('') }}
                  style={{ background:C.gold, color:'#000', border:'none', padding:'8px 14px', fontFamily:'monospace', fontWeight:700, fontSize:'11px', cursor:'pointer' }}>ADD</button>
              </div>
            </div>
        ...
        {tab==='rss' && (
          <div style={{ maxWidth:700 }}>
            <div style={{ fontFamily:'monospace', fontSize:'10px', color:C.gold, letterSpacing:'0.15em', marginBottom:14 }}>NEWS RSS FEEDS</div>
            {rssFeeds.map((url,i)=>(
              <div key={i} style={{ background:C.bg2, border:`1px solid ${C.border}`, padding:'10px 14px', marginBottom:6, display:'flex', alignItems:'center', gap:10 }}>
                <span style={{ fontFamily:'monospace', fontSize:'11px', color:C.text, flex:1, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{url}</span>
                <a href={url} target="_blank" rel="noreferrer" style={{ fontFamily:'monospace', fontSize:'9px', color:C.blue, textDecoration:'none', flexShrink:0 }}>TEST ↗</a>
                <button onClick={()=>setRssFeeds(p=>p.filter((_,j)=>j!==i))} style={{ background:'none', border:'none', color:C.red, cursor:'pointer', fontFamily:'monospace', fontSize:'11px' }}>✕</button>
              </div>
            ))}
            <div style={{ display:'flex', gap:8, marginTop:12, marginBottom:16 }}>
              <input value={newRss} onChange={e=>setNewRss(e.target.value)} placeholder="https://example.com/feed.xml"
                style={{ flex:1, background:C.bg2, border:`1px solid ${C.border}`, color:C.text, padding:'9px 12px', fontFamily:'monospace', fontSize:'11px' }} />
              <button onClick={()=>{ if(!newRss)return; setRssFeeds(p=>[...p,newRss.trim()]); setNewRss('') }}
                style={{ background:C.gold, color:'#000', border:'none', padding:'9px 16px', fontFamily:'monospace', fontWeight:700, fontSize:'11px', cursor:'pointer' }}>ADD</button>
            </div>
        ...
        {tab==='newsletter' && (
          <div style={{ maxWidth:700 }}>
            <div style={{ fontFamily:'monospace', fontSize:'10px', color:C.gold, letterSpacing:'0.15em', marginBottom:14 }}>NEWSLETTER COMPOSE</div>
            <p style={{ fontFamily:'monospace', fontSize:'11px', color:C.muted, marginBottom:20, lineHeight:1.7 }}>Compose and send newsletters to all subscribers. Requires RESEND_API_KEY and RESEND_AUDIENCE_ID.</p>
            <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
              <div>
                <label style={{ fontFamily:'monospace', fontSize:'10px', color:C.muted, display:'block', marginBottom:6 }}>SUBJECT LINE</label>
                <input value={newsletter.subject} onChange={e=>setNewsletter(p=>({...p,subject:e.target.value}))} placeholder="DownRange Weekly Brief — This Week in 2A"
                  style={{ width:'100%', background:C.bg2, border:`1px solid ${C.border}`, color:C.text, padding:'10px 12px', fontFamily:'monospace', fontSize:'12px', boxSizing:'border-box' }} />
              </div>
              <div>
                <label style={{ fontFamily:'monospace', fontSize:'10px', color:C.muted, display:'block', marginBottom:6 }}>BODY (Markdown or plain text)</label>
                <textarea value={newsletter.body} onChange={e=>setNewsletter(p=>({...p,body:e.target.value}))} rows={12} placeholder="Write your newsletter content here..."
                  style={{ width:'100%', background:C.bg2, border:`1px solid ${C.border}`, color:C.text, padding:'10px 12px', fontFamily:'monospace', fontSize:'12px', boxSizing:'border-box', resize:'vertical' }} />
              </div>
              ...
              {sendResult && <div style={{ fontFamily:'monospace', fontSize:'12px', color:sendResult.startsWith('✓')?C.green:C.red, padding:'10px', background:C.bg2, border:`1px solid ${C.border}` }}>{sendResult}</div>}
            </div>
          </div>
        )}
        ...
        {tab==='identity' && (
          <div style={{ maxWidth:600 }}>
            <div style={{ fontFamily:'monospace', fontSize:'10px', color:C.gold, letterSpacing:'0.15em', marginBottom:14 }}>SITE IDENTITY</div>
            <p style={{ fontFamily:'monospace', fontSize:'11px', color:C.muted, marginBottom:20, lineHeight:1.7 }}>Configure site-wide text and branding. Changes saved to localStorage and Sanity siteConfig.</p>
            {[['Site Name','siteName','DownRange'],['Tagline','tagline',"America's Firearms Intelligence Hub"],['Footer Text','footerText','Proudly Independent · Pro-Second Amendment']].map(([label,key,placeholder])=>(
              <div key={key} style={{ marginBottom:14 }}>
                <label style={{ fontFamily:'monospace', fontSize:'10px', color:C.muted, display:'block', marginBottom:6 }}>{label.toUpperCase()}</label>
                <input value={identity[key]||''} onChange={e=>setIdentity(p=>({...p,[key]:e.target.value}))} placeholder={placeholder}
                  style={{ width:'100%', background:C.bg2, border:`1px solid ${C.border}`, color:C.text, padding:'10px 12px', fontFamily:'monospace', fontSize:'12px', boxSizing:'border-box' }} />
              </div>
            ))}
            <div style={{ marginBottom:14 }}>
              <label style={{ fontFamily:'monospace', fontSize:'10px', color:C.muted, display:'block', marginBottom:6 }}>BRAND COLOR (Gold)</label>
              <div style={{ display:'flex', alignItems:'center', gap:12 }}>
                <div style={{ width:40, height:40, background:'#C8922A', border:`1px solid ${C.border}` }} />
                <span style={{ fontFamily:'monospace', fontSize:'12px', color:C.muted }}>#C8922A (hardcoded in globals.css — edit CSS var --gold)</span>
              </div>
            </div>
        ...
        {tab==='keys' && (
          <div style={{ maxWidth:700 }}>
            <div style={{ fontFamily:'monospace', fontSize:'10px', color:C.gold, letterSpacing:'0.15em', marginBottom:8 }}>API KEY STATUS</div>
            <p style={{ fontFamily:'monospace', fontSize:'11px', color:C.muted, marginBottom:18, lineHeight:1.7 }}>All keys are set in Vercel → Settings → Environment Variables. Status is read from the live server.</p>
            {API_KEYS.map(k=>{
              ...
              return (
                <div key={k.key} style={{ background:C.bg2, border:`1px solid ${C.border}`, padding:'10px 14px', marginBottom:7, display:'flex', alignItems:'center', gap:12 }}>
                  <div style={{ flex:1 }}>
                    ...
                    <div style={{ fontFamily:'monospace', fontSize:'9px', color:C.muted, marginTop:2 }}>{k.hint}</div>
                  </div>
                  <div style={{ display:'flex', alignItems:'center', gap:8, flexShrink:0 }}>
                    {k.required&&<span style={{ fontFamily:'monospace', fontSize:'8px', color:C.red, background:'#1A0000', padding:'2px 6px' }}>REQUIRED</span>}
                    <span style={{ width:7, height:7, borderRadius:'50%', background:ok==null?'#374151':ok?C.green:C.red, display:'inline-block' }} />
                    <span style={{ fontFamily:'monospace', fontSize:'10px', color:ok==null?'#374151':ok?C.green:C.red, minWidth:60 }}>{ok==null?'—':ok?'SET ✓':'MISSING ✗'}</span>
                  </div>
                </div>
              )
            })}
            <button onClick={checkStatus} style={{ marginTop:10, background:'none', border:`1px solid ${C.border}`, color:C.muted, padding:'8px 18px', fontFamily:'monospace', fontSize:'11px', cursor:'pointer' }}>REFRESH STATUS</button>
          </div>
        )}
        ...
        {tab==='settings' && (
          <div style={{ maxWidth:600 }}>
            <div style={{ fontFamily:'monospace', fontSize:'10px', color:C.gold, letterSpacing:'0.15em', marginBottom:20 }}>SITE SETTINGS</div>
            <div style={{ background:C.bg2, border:`1px solid ${C.border}`, padding:'20px', marginBottom:14 }}>
              <div style={{ fontFamily:'monospace', fontSize:'12px', color:C.text, marginBottom:10, fontWeight:600 }}>Breaking Alert Urgency Threshold</div>
              <p style={{ fontFamily:'monospace', fontSize:'10px', color:C.muted, marginBottom:12, lineHeight:1.6 }}>Articles scored ≥ this value trigger a breaking alert and Discord ping.</p>
            ...
            <div style={{ background:C.bg2, border:`1px solid ${C.border}`, padding:'20px', marginBottom:14 }}>
              <div style={{ fontFamily:'monospace', fontSize:'12px', color:C.text, marginBottom:14, fontWeight:600 }}>System Information</div>
