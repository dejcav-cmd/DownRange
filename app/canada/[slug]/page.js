...
function getFallback(article) {
  ...
  if (/rifle|carbine|ar.?15|semi.auto/.test(t)) return '/img/photos/rifle.jpg'
  if (/pistol|handgun|revolver|ccw|carry/.test(t)) return '/img/photos/pistol.jpg'
  if (/ammo|ammunition|cartridge/.test(t)) return '/img/photos/ammo.jpg'
  if (/hunt|deer|elk|game/.test(t)) return '/img/photos/hunting.jpg'
  if (/suppressor|silencer/.test(t)) return '/img/photos/suppressor.jpg'
...
async function getArticle(slug) {
  return sanity.fetch(
    `*[_type == "canadaContent" && type == "article" && active == true && slug.current == $slug][0] {
...
async function getRelated(slug) {
  return sanity.fetch(
    `*[_type == "canadaContent" && type == "article" && active == true && slug.current != $slug]
...
export async function generateMetadata({ params }) {
  ...
  const url = `https://downrangeco.com/canada/${params.slug}`
  return {
...
const TAG_COLORS = {
  LAW:    { color: '#60A5FA', bg: '#001a2a', label: 'LAW'    },
  POLICY: { color: '#C8922A', bg: '#1a1000', label: 'POLICY' },
  GUIDE:  { color: '#34D399', bg: '#001a0a', label: 'GUIDE'  },
  ALERT:  { color: '#EF4444', bg: '#2a0000', label: 'ALERT'  },
  NEWS:   { color: '#9CA3AF', bg: '#1a1f2e', label: 'NEWS'   },
}

export default async function CanadaArticlePage({ params }) {
  ...
  const pageUrl  = `https://downrangeco.com/canada/${params.slug}`
  ...
  return (
    <>
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
        ...
        author: [{ '@type': 'Person', name: article.author || 'DownRange' }],
        publisher: {
        ...
        <div style={{ background: 'transparent', borderBottom: '1px solid var(--border)', marginTop: '-80px', position: 'relative', zIndex: 1 }}>
          <div style={{ maxWidth: 900, margin: '0 auto', padding: '2rem 1.5rem 2.5rem' }}>
            ...
            <div style={{ display: 'flex', gap: 6, alignItems: 'center', marginBottom: '1.5rem', fontSize: '0.72rem', fontFamily: "'IBM Plex Mono',monospace" }}>
              ...
              <a href="/canada" style={{ color: '#4B5563', textDecoration: 'none' }}>🇨🇦 CANADA</a>
              <span style={{ color: '#2D3748' }}>›</span>
            ...
            <div style={{ display: 'flex', gap: '1.25rem', alignItems: 'center', flexWrap: 'wrap', fontSize: '0.78rem', color: '#4B5563', fontFamily: "'IBM Plex Mono',monospace" }}>
              <span>{article.author || 'DownRange Canada'}</span>
              <span style={{ color: '#2D3748' }}>|</span>
        ...
        <div style={{ maxWidth: 900, margin: '0 auto', padding: '2rem 1.5rem', display: 'grid', gridTemplateColumns: '1fr 280px', gap: '2.5rem' }}>
              ...
              <>
                ...
                {article.sourceUrl && (
                  <div style={{ margin: '2.5rem 0 0', padding: '1.25rem 1.5rem', background: 'rgba(200,146,42,0.06)', border: '1px solid rgba(200,146,42,0.25)', borderLeft: '4px solid #C8922A', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
                    <div>
                      <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: '0.65rem', color: '#C8922A', letterSpacing: '0.15em', fontWeight: 700, marginBottom: 4 }}>ORIGINAL SOURCE</div>
                      <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: '0.8rem', color: '#6B7280', lineHeight: 1.6 }}>Written by DownRange based on the original. Read the primary source for additional detail.</div>
                    </div>
                    <a href={article.sourceUrl} target="_blank" rel="noopener noreferrer"
              ...
              <div>
                ...
                {article.sourceUrl && (
                  <div style={{ marginTop: '2rem', padding: '1.5rem 2rem', background: '#0D1117', border: '1px solid #C8922A30', borderLeft: '4px solid #C8922A' }}>
                    <div style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: '0.65rem', color: '#C8922A', letterSpacing: '0.15em', fontWeight: 700, marginBottom: 8 }}>ORIGINAL SOURCE</div>
                    <p style={{ color: '#6B7280', marginBottom: '1rem', fontSize: '0.875rem', lineHeight: 1.6 }}>Written by DownRange based on the original article.</p>
                    <a href={article.sourceUrl} target="_blank" rel="noopener noreferrer"
          ...
          <aside style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            <div style={{ background: '#111318', border: '1px solid var(--border)', padding: '1.25rem' }}>
              <div style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: '0.68rem', color: '#C8922A', letterSpacing: '0.12em', marginBottom: '1rem', fontWeight: 700 }}>🇨🇦 MORE FROM CANADA</div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                {related.map(a => (
                  <a key={a._id} href={`/canada/${a.slug?.current}`} style={{ textDecoration: 'none', display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                    <div style={{ width: 54, height: 40, flexShrink: 0, overflow: 'hidden', background: '#001a2a' }}>
              ...
              <a href="/canada" style={{ display: 'block', textAlign: 'center', marginTop: '1rem', color: '#C8922A', textDecoration: 'none', fontFamily: "'IBM Plex Mono',monospace", fontSize: '0.68rem', letterSpacing: '0.1em' }}>
                ALL CANADA →
              </a>
            </div>
          </aside>
