  ...
  return (
    <>
      ...
      <div style={{ background:'#0A0B0C' }}>
        ...
        {/* Hero */}
        <div style={{ padding:'80px 0 60px', borderBottom:'1px solid #1F2428' }}>
          <div className="container" style={{ maxWidth:900 }}>
            <div style={{ fontFamily:'monospace', fontSize:'10px', color:'#C8922A', letterSpacing:'0.15em', marginBottom:'16px' }}>PRESS KIT · MEDIA RESOURCES · {new Date().getFullYear()}</div>
            <h1 style={{ fontFamily:"'Bebas Neue', sans-serif", fontSize:'clamp(3rem,8vw,6rem)', color:'#F5F5F3', letterSpacing:'0.02em', lineHeight:0.95, marginBottom:'24px' }}>
              DOWN<br/><span style={{ color:'#C8922A' }}>RANGE</span>
            </h1>
            <p style={{ fontSize:'18px', color:'#94A3B8', lineHeight:1.8, maxWidth:620, marginBottom:'32px' }}>
              America's most comprehensive AI-powered firearms intelligence platform. We aggregate, analyze, and deliver 2A news and law intelligence to 100 million American gun owners — faster and more comprehensively than any other source.
            </p>
            <div style={{ display:'flex', gap:'12px', flexWrap:'wrap' }}>
              <Link href="/contact" style={{ background:'#C8922A', color:'#000', padding:'12px 28px', fontFamily:'monospace', fontWeight:700, fontSize:'13px', textDecoration:'none', letterSpacing:'0.05em' }}>
                PRESS INQUIRY →
              </Link>
              <a href="mailto:press@downrangeco.com" style={{ background:'transparent', color:'#C8922A', border:'1px solid #C8922A', padding:'12px 28px', fontFamily:'monospace', fontWeight:700, fontSize:'13px', textDecoration:'none', letterSpacing:'0.05em' }}>
                press@downrangeco.com
              </a>
            </div>
          </div>
        </div>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:'24px' }}>
              {STATS.map(s=>(
                <div key={s.num} style={{ textAlign:'center' }}>
                  ...
                  <div style={{ fontFamily:'monospace', fontSize:'11px', color:'#F0EDE6', letterSpacing:'0.08em', marginTop:'6px' }}>{s.label.toUpperCase()}</div>
          ...
          <div className="container" style={{ maxWidth:900 }}>
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'48px' }}>
              <div>
                ...
                <p style={{ fontSize:'14px', color:'#94A3B8', lineHeight:1.85, marginBottom:'16px' }}>
              ...
              <div>
                ...
                <div style={{ display:'flex', flexDirection:'column', gap:'6px' }}>
                  {COVERAGE_AREAS.map(c=>(
                    <div key={c} style={{ display:'flex', gap:'10px', alignItems:'flex-start', padding:'6px 0', borderBottom:'1px solid #1F2428' }}>
                      <span style={{ color:'#C8922A', flexShrink:0, marginTop:'1px' }}>◈</span>
                      <span style={{ fontFamily:'monospace', fontSize:'12px', color:'#94A3B8' }}>{c}</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div className="container" style={{ maxWidth:900 }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'12px' }}>
              {BRAND_GUIDELINES.map((g,i)=>(
                <div key={i} style={{ background:'#111318', border:'1px solid #1F2428', padding:'16px 18px' }}>
                  <div style={{ fontFamily:'monospace', fontSize:'9px', color:'#34D399', letterSpacing:'0.12em', marginBottom:'6px' }}>✓ DO</div>
                  <div style={{ fontFamily:'monospace', fontSize:'12px', color:'#D1D5DB', marginBottom:'10px', lineHeight:1.5 }}>{g.do}</div>
                  <div style={{ fontFamily:'monospace', fontSize:'9px', color:'#EF4444', letterSpacing:'0.12em', marginBottom:'6px' }}>✗ DON'T</div>
                  <div style={{ fontFamily:'monospace', fontSize:'12px', color:'#6B7280', lineHeight:1.5 }}>{g.dont}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="container" style={{ maxWidth:900 }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:'16px', marginBottom:'32px' }}>
              {[
                ...
                <div key={c.name}>
                  <div style={{ height:60, background:c.hex, border:'1px solid #1F2428', marginBottom:'8px' }} />
                  <div style={{ fontFamily:'monospace', fontSize:'11px', color:'#F0EDE6', marginBottom:'2px' }}>{c.name}</div>
                  <div style={{ fontFamily:'monospace', fontSize:'10px', color:'#C8922A', marginBottom:'4px' }}>{c.hex}</div>
                  <div style={{ fontFamily:'monospace', fontSize:'9px', color:'#4B5563', lineHeight:1.5 }}>{c.on}</div>
                </div>
              ))}
            </div>
            <div style={{ display:'grid', gridTemplateColumns:'repeat(2,1fr)', gap:'16px' }}>
              {[
                ...
                <div key={f.name} style={{ background:'#111318', border:'1px solid #1F2428', padding:'16px 20px' }}>
                  <div style={{ fontFamily:'monospace', fontSize:'10px', color:'#4B5563', marginBottom:'8px' }}>TYPEFACE</div>
                  <div style={{ fontFamily:'monospace', fontSize:'13px', color:'#F0EDE6', fontWeight:700, marginBottom:'4px' }}>{f.name}</div>
                  <div style={{ fontFamily:'monospace', fontSize:'11px', color:'#4B5563', marginBottom:'12px' }}>{f.use}</div>
          ...
          <div className="container" style={{ maxWidth:900 }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'16px' }}>
              {[
                ...
                <div key={c.role} style={{ background:'#111318', border:'1px solid #1F2428', padding:'20px' }}>
                  <div style={{ fontFamily:'monospace', fontSize:'10px', color:'#C8922A', letterSpacing:'0.1em', marginBottom:'8px', fontWeight:700 }}>{c.role.toUpperCase()}</div>
                  <a href={`mailto:${c.email}`} style={{ fontFamily:'monospace', fontSize:'13px', color:'#60A5FA', display:'block', marginBottom:'8px', textDecoration:'none' }}>{c.email}</a>
                  <p style={{ fontFamily:'monospace', fontSize:'11px', color:'#4B5563', lineHeight:1.6 }}>{c.desc}</p>
                </div>
              ))}
            </div>
            <div style={{ marginTop:'24px', padding:'20px 24px', background:'#111318', border:'1px solid #1F2428', fontFamily:'monospace', fontSize:'12px', color:'#4B5563', lineHeight:1.7 }}>
