...
// ── LIVE NEWS GRID — client component, auto-refreshes every 5 min ────────────
// Shows photos on every card, magazine-style layout
'use client'
function LiveNewsGrid({ articles: initialArticles }) {
  const [articles, setArticles] = React.useState(initialArticles || [])
  const [newCount, setNewCount] = React.useState(0)
  const [lastRefresh, setLastRefresh] = React.useState(null)
  const [refreshing, setRefreshing] = React.useState(false)

  // Firearm fallback images — always show something relevant
  const FALLBACKS = {
  ...
  React.useEffect(() => {
    const refresh = async () => {
      try {
        ...
        if (data.articles?.length > 0) {
          const prev = new Set(articles.map(a => a._id))
          const incoming = data.articles
          const fresh = incoming.filter(a => !prev.has(a._id)).length
    ...
    const timer = setInterval(refresh, 5 * 60 * 1000) // 5 min
  ...
  return (
    <section style={{ padding:'36px 0', borderBottom:'1px solid var(--border)', background:'var(--bg)' }}>
      <style>{`
        ...
        .story-new { animation: fadeSlideIn 0.4s ease; }
      ...
      <div className="container">
        ...
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:20 }}>
          <div style={{ display:'flex', alignItems:'center', gap:12 }}>
            ...
            {refreshing && (
              <span style={{ width:10, height:10, border:'2px solid #334155', borderTopColor:'#C8922A', borderRadius:'50%', display:'inline-block' }} className="spinner" />
            )}
            {lastRefresh && (
          ...
          <a href="/news" style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:10, color:'#C8922A', textDecoration:'none', letterSpacing:'0.06em' }}>
            ALL NEWS →
          </a>
        </div>

        {/* Magazine grid — 3 cols with varied sizes */}
        <div style={{ display:'grid', gridTemplateColumns:'repeat(12, 1fr)', gap:10 }}>
          {displayArticles.slice(0, 8).map((a, i) => {
            const img   = getImage(a)
            const cc    = CAT_C[a.category] || '#9ca3af'
            const cl    = CAT_L[a.category] || a.category?.toUpperCase()
            const slug  = a.slug?.current || a._id
            const href  = `/news/${slug}`
            const isHot = a.urgencyScore >= 8
            // Vary span: first card is big (6 cols), others are 3 cols (2 per row)
            ...
            return (
              <a key={a._id || i}
                href={href}
                className="story-card story-new"
                style={{
              ...
              >
                ...
                <div style={{ position:'relative', height:imgH, overflow:'hidden', background:'#0d1117' }}>
                  <img
                    ...
                    onError={e => {
                      const fb = FALLBACKS[a.category] || FALLBACKS.news
                      if (e.target.src !== fb) e.target.src = fb
                    }}
                  />
                  {/* Gradient overlay */}
                  <div style={{ position:'absolute', inset:0, background:'linear-gradient(0deg, rgba(9,9,11,0.85) 0%, rgba(9,9,11,0.2) 60%, transparent 100%)' }} />
                  {/* Category pill */}
                  <div style={{ position:'absolute', top:10, left:10, display:'flex', gap:6, alignItems:'center' }}>
                    <span style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:8, fontWeight:700, color: cc === '#C8922A' ? '#000' : '#fff', background:cc, padding:'2px 8px', letterSpacing:'0.08em' }}>
                      {cl}
                    </span>
                    {isHot && (
                      <span style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:8, color:'#fff', background:'#B91C1C', padding:'2px 7px', display:'flex', alignItems:'center', gap:3 }}>
                        <span style={{ width:4, height:4, borderRadius:'50%', background:'#fff', display:'inline-block', animation:'pulse 1s infinite' }} />
                        HOT
                      </span>
                    )}
                  </div>
                  {/* Title over image on large cards */}
                  {i < 2 && (
                    <div style={{ position:'absolute', bottom:0, left:0, right:0, padding:'14px 16px' }}>
                      <div style={{ fontFamily:"'Bebas Neue',cursive", fontSize: i===0 ? '1.5rem' : '1.2rem', color:'#F0EDE6', letterSpacing:'0.03em', lineHeight:1.1, marginBottom:6 }}>
                        {a.title}
                      </div>
                      <div style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:9, color:'#94a3b8' }}>
                        {a.source} · {timeAgoClient(a.publishedAt)}
                      </div>
                    </div>
                  )}
                </div>

                {/* Text below image — for smaller cards */}
                {i >= 2 && (
                  <div style={{ padding:'12px 14px' }}>
                    <div style={{ fontFamily:"'Barlow Condensed',sans-serif", fontSize:14, fontWeight:700, color:'var(--foreground)', lineHeight:1.25, marginBottom:6, display:'-webkit-box', WebkitLineClamp:2, WebkitBoxOrient:'vertical', overflow:'hidden' }}>
                      {a.title}
                    </div>
                    {(a.excerpt || a.summary) && (
                      <div style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:9, color:'#475569', lineHeight:1.5, marginBottom:8, display:'-webkit-box', WebkitLineClamp:2, WebkitBoxOrient:'vertical', overflow:'hidden' }}>
                        {(a.excerpt || a.summary)?.slice(0, 100)}
                      </div>
                    )}
                    <div style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:9, color:'#334155' }}>
                      {a.source} · {timeAgoClient(a.publishedAt)}
        ...
        {/* Second row — 4 columns with image+text compact cards */}
        {displayArticles.length > 8 && (
          <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:10, marginTop:10 }}>
            {displayArticles.slice(8, 12).map((a, i) => {
              const img  = getImage(a)
              const cc   = CAT_C[a.category] || '#9ca3af'
              const slug = a.slug?.current || a._id
              return (
                ...
                >
                  ...
                  <div style={{ flex:1, minWidth:0 }}>
                    <div style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:8, color:cc, marginBottom:4, letterSpacing:'0.06em' }}>
                      {a.category?.toUpperCase()}
                    </div>
                    <div style={{ fontFamily:"'Barlow Condensed',sans-serif", fontSize:13, fontWeight:700, color:'var(--foreground)', lineHeight:1.2, marginBottom:4, display:'-webkit-box', WebkitLineClamp:2, WebkitBoxOrient:'vertical', overflow:'hidden' }}>
                      {a.title}
                    </div>
                    <div style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:8, color:'#334155' }}>
                      {a.source} · {timeAgoClient(a.publishedAt)}
                    </div>
                  </div>
                </a>
