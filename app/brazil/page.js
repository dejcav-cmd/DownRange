...
export default async function BrazilPage({ searchParams }) {
  ...
  const [items, breaking] = await Promise.all([
    sanity.fetch(`*[_type=="brazilContent"] | order(publishedAt desc) {
      _id, type, title, slug, status, impact, effectiveDate, summary,
      detail, sourceUrl, abbr, rating, highlights, body, imageUrl,
      tag, readMins, author, brlPrice, usdEquiv, availability, trend,
    ...
    <>
      ...
      <div style={{ background:'var(--bg2)', borderBottom:'1px solid var(--border)', padding:'52px 0 36px', position:'relative', overflow:'hidden' }}>
        ...
        <div style={{ position:'absolute', right:0, top:0, bottom:0, width:'40%', overflow:'hidden', opacity:0.04, pointerEvents:'none' }}>
          <div style={{ fontFamily:"'Bebas Neue',cursive", fontSize:'18vw', color:'#009C3B', lineHeight:0.85, textAlign:'right', paddingRight:20, paddingTop:10 }}>BR</div>
        </div>
      ...
      <div style={{ background:'var(--bg2)', borderBottom:'1px solid var(--border)', position:'sticky', top:'60px', zIndex:20 }}>
        <div className="container">
          <div style={{ display:'flex', gap:0, overflowX:'auto', alignItems:'stretch' }}>
            ...
            {[
              { href:'/brazil#leis',    label:'⚖ Leis Federais' },
              { href:'/brazil#cac',     label:'🎯 Guia CAC' },
              { href:'/brazil#estados', label:'🗺 Estados' },
              { href:'/brazil#municao', label:'🔴 Munição' },
            ].map(l => (
            ...
            <div className="sidebar">
              <div>
                ...
                {(leis.length > 0 ? leis : [
                  ...
                  { _id:'l4', title:'SIGMA — Registro Exército', status:'OBRIGATÓRIO', effectiveDate:'Contínuo' },
                ]).slice(0,5).map(l => (
              ...
              <div>
                ...
                {(estados.length > 0 ? estados : [
                  ...
                  { _id:'e3', abbr:'MG', title:'Minas Gerais',      rating:'C+', color:'#f59e0b' },
                  { _id:'e4', abbr:'SP', title:'São Paulo',         rating:'D',  color:'#dc2626' },
                  { _id:'e5', abbr:'RJ', title:'Rio de Janeiro',    rating:'D',  color:'#dc2626' },
                ]).slice(0,7).map(e => (
                  <div key={e._id} style={{ display:'flex', alignItems:'center', gap:10, padding:'8px 0', borderBottom:'1px solid var(--border)' }}>
                    <div style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:10, fontWeight:700, color:'var(--gold)', minWidth:28 }}>{e.abbr}</div>
      ...
      {/* ── REFERENCE SECTIONS (Leis / CAC / Estados / Munição) ── */}
