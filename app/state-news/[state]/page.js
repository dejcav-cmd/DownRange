        ...
        <div className="container">
          ...
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'20px', flexWrap:'wrap', gap:12 }}>
            ...
            <div style={{ display:'flex', gap:'6px', flexWrap:'wrap' }}>
              {categories.map(c => (
                <a key={c} href={`/state-news/${params.state}?cat=${c==='all'?'':c}&sort=${sort}`}
                  style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:'10px', padding:'4px 12px', border:'1px solid var(--border)',
            ...
            <div style={{ display:'flex', gap:'6px' }}>
              <span style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:'10px', color:'#4B5563', alignSelf:'center' }}>SORT:</span>
          ...
          {allNews.length === 0 ? (
            ...
            <div style={{ display:'grid', gridTemplateColumns:'2fr 1fr', gap:'32px' }}>
                ...
                {allNews.map((article, i) => {
                  ...
                  return (
                    <a key={article._id || i} href={href}
                      ...
                      style={{ display:'block', textDecoration:'none', background:'#111318', border:'1px solid var(--border)', padding:'14px 18px', borderLeft:`3px solid ${i < 3 ? catColor : '#1F2428'}` }}>
                      {/* Top meta row */}
                      <div style={{ display:'flex', gap:'10px', alignItems:'center', marginBottom:'6px', flexWrap:'wrap' }}>
                        {article.category && (
                          <span style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:'9px', color:catColor, letterSpacing:'0.12em' }}>
                            {article.category.toUpperCase()}
                          </span>
                        )}
                        {article.urgencyScore >= 8 && (
                          <span style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:'9px', color:'#EF4444', background:'#1A0000', padding:'1px 6px' }}>
                            ⚡ {article.urgencyScore}/10
                          </span>
                        )}
                        {article.fromRSS && (
                          <span style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:'9px', color:'#34D399', background:'#001A0A', padding:'1px 6px' }}>
                            RSS LIVE
                          </span>
                        )}
                      </div>
                      ...
                      {(article.summary || article.excerpt) && (
                        <p style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:'11px', color:'#4B5563', lineHeight:1.6, marginBottom:'8px' }}>
                          {(article.summary || article.excerpt)?.slice(0, 160)}
                        </p>
                      )}
                      <div style={{ display:'flex', gap:'12px', fontFamily:"'IBM Plex Mono',monospace", fontSize:'10px', color:'#374151' }}>
              ...
              <aside>
                {/* State law summary */}
                <div style={{ background:'#111318', border:'1px solid var(--border)', padding:'18px', marginBottom:'16px' }}>
                  ...
                  <Link href={`/laws?tab=state&state=${abbr}`}
                    style={{ display:'block', fontFamily:"'IBM Plex Mono',monospace", fontSize:'11px', color:'#60A5FA', textDecoration:'none' }}>
                    {stateName} bills in tracker →
                  </Link>
                </div>

                {/* Other states */}
                <div style={{ background:'#111318', border:'1px solid var(--border)', padding:'18px' }}>
                  <div style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:'10px', color:'#C8922A', letterSpacing:'0.12em', marginBottom:'12px', fontWeight:700 }}>
                    OTHER STATE NEWS
                  </div>
                  <div style={{ display:'flex', flexWrap:'wrap', gap:'6px' }}>
                    {['TX','CA','FL','WA','IL','NY','AZ','CO','GA','OH'].filter(s=>s!==abbr).map(s=>(
                      <a key={s} href={`/state-news/${s.toLowerCase()}`}
                        style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:'10px', color:'#6B7280', background:'#0D1117', border:'1px solid var(--border)', padding:'3px 8px', textDecoration:'none' }}>
                        {s}
                      </a>
                    ))}
                  </div>
