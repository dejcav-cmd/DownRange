...
const DEFAULT_CHAINS = {
  ...
  // Fast/bulk operations — cheapest (summaries, dedup, social — NOT full articles)
  fast:     'glm:glm-4.5-air,anthropic:claude-haiku-4-5-20251001',
  // generation — newsletter drafts, admin one-offs. Was missing → fell to 'default' → Sonnet surprise.
  // Haiku primary: drafts don't need flagship reasoning; Sonnet only if Haiku fails.
  generation: 'anthropic:claude-haiku-4-5-20251001,anthropic:claude-sonnet-4-5-20251022',
  // social — 300-token posts, 5×/day. GLM air is 10× cheaper than Haiku for trivial generation.
  social:   'glm:glm-4.5-air,anthropic:claude-haiku-4-5-20251001',
...
// Strip markdown code fences that some models (GLM, etc.) add despite instructions.
// Safe for both HTML and plain-text responses — backticks never appear in real article HTML.
export function stripMarkdownFences(text) {
  if (!text) return text
  return text
    .replace(/^```[a-z]*\r?\n?/im, '')  // opening fence e.g. ```html or ```
    .replace(/\r?\n?```\s*$/im, '')       // closing fence
    .trim()
}

// Convenience — returns just the text string, with markdown fences stripped
export async function callAIText(opts) {
  const r = await callAI(opts)
  return stripMarkdownFences(r.text)
}
