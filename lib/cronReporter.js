...
function getTroubleshootingGuide(jobId) {
  const guides = {
    ...
    'quality-rewrite':{ what: 'Quality scanner + AI rewrite — scans all content for AI phrases/short bodies, rewrites failing items', checks: ['ANTHROPIC_API_KEY set?', 'SANITY_API_TOKEN set?'], likelyCause: 'Claude API timeout, rate limit, or Sanity write error. Check ANTHROPIC_API_KEY.' },
    'image-fix':      { what: 'Image fixer — fetches OG images from source URLs, assigns real photo fallbacks', checks: ['SANITY_API_TOKEN set?', 'Article source URLs reachable?'], likelyCause: 'Sanity write token expired or all source URLs timing out.' },
