...
export function generateNewsletterHTML(data = {}, isTest = false) {
  const {
    ...
    ammo    = [],
    nfa     = null,
    state   = null,   // stateProfile doc { name, abbr, rating, recentBills, summary } — personalizes lead block
    outlook = '',      // AI "What to Watch Next" teaser, 2 sentences
  ...
  // Weekly issue number (was daily — newsletter moved to a once-a-week send)
  const issueNum = Math.floor((now - new Date('2026-01-01')) / (1000*60*60*24*7)) + 1
  ...
  // ── YOUR STATE THIS WEEK (personalization) ─────────────────────────────────
  const stateBlock = state ? (() => {
    const bill = (state.recentBills && state.recentBills[0]) || null
    return `
    <!-- YOUR STATE THIS WEEK -->
    <tr><td style="background:#111111;border-left:4px solid #C8922A;padding:36px 36px;">
      <p style="margin:0 0 6px;font-family:'IBM Plex Mono','Courier New',monospace;font-size:10px;font-weight:700;letter-spacing:.32em;color:#C8922A;">&#9658;&nbsp; YOUR STATE THIS WEEK</p>
      <h2 style="margin:0 0 14px;font-family:'Anton',Impact,'Arial Black',sans-serif;font-size:28px;letter-spacing:.03em;text-transform:uppercase;color:#F0EDE6;">${safe(state.name||state.abbr)}${state.rating ? ` <span style="font-size:16px;color:#4B5563;">&mdash; Freedom Grade ${safe(state.rating)}</span>` : ''}</h2>
      ${bill ? `
      <p style="margin:0 0 6px;font-family:'IBM Plex Mono','Courier New',monospace;font-size:10px;letter-spacing:.14em;color:#374151;">${safe((bill.status||'BILL WATCH').toUpperCase())}</p>
      <p style="margin:0 0 16px;font-family:Georgia,serif;font-style:italic;font-size:16px;line-height:1.6;color:#9CA3AF;">${safe(trunc(bill.title||'',180))}</p>
      ${bill.url ? `<a href="${bill.url}" style="font-family:'IBM Plex Mono','Courier New',monospace;font-size:10px;color:#C8922A;text-decoration:none;font-weight:700;letter-spacing:.12em;">TRACK THIS BILL &rarr;</a>` : ''}
      ` : `<p style="margin:0 0 16px;font-family:Arial,sans-serif;font-size:14px;line-height:1.7;color:#6B7280;">No new bill activity in ${safe(state.name||state.abbr)} this week &mdash; laws unchanged since your last brief.</p>`}
      <p style="margin:16px 0 0;">
        <a href="https://www.downrangeco.com/state-hub/${safe(state.abbr)}" style="font-family:'IBM Plex Mono','Courier New',monospace;font-size:10px;color:#C8922A;text-decoration:none;font-weight:700;letter-spacing:.12em;">FULL ${safe(state.abbr)} PROFILE &rarr;</a>
      </p>
    </td></tr>`
  })() : `
    <!-- SET YOUR STATE CTA -->
    <tr><td style="background:#111111;border-left:4px solid #C8922A;padding:32px 36px;">
      <p style="margin:0 0 8px;font-family:'IBM Plex Mono','Courier New',monospace;font-size:10px;font-weight:700;letter-spacing:.28em;color:#C8922A;">&#9658;&nbsp; GET STATE-SPECIFIC INTEL</p>
      <p style="margin:0 0 14px;font-family:Arial,sans-serif;font-size:14px;line-height:1.7;color:#9CA3AF;">Tell us your state and every brief leads with the laws, bills, and reciprocity news that actually affect you.</p>
      <a href="https://www.downrangeco.com/laws" style="font-family:'IBM Plex Mono','Courier New',monospace;font-size:10px;color:#C8922A;text-decoration:none;font-weight:700;letter-spacing:.12em;">SET YOUR STATE &rarr;</a>
    </td></tr>`

  // ── WHAT TO WATCH NEXT (AI teaser, one call per send) ──────────────────────
  const outlookBlock = outlook ? `
    <tr><td style="padding:0 40px;background:#0a0a0a;">
      <div style="height:1px;background:rgba(200,146,42,0.2);"></div>
    </td></tr>
    <tr><td style="background:#0f0f0f;padding:40px 40px;">
      <p style="margin:0 0 10px;font-family:'IBM Plex Mono','Courier New',monospace;font-size:10px;font-weight:700;letter-spacing:.32em;text-transform:uppercase;color:#C8922A;">&#9658;&nbsp; WHAT TO WATCH NEXT</p>
      <p style="margin:0;font-family:Georgia,serif;font-style:italic;font-size:16px;line-height:1.7;color:#9CA3AF;max-width:520px;">${safe(outlook)}</p>
    </td></tr>` : ''
