...
/**
 * Returns the DownRange group ID (numeric string).
 * - If MAILERLITE_GROUP_ID is a numeric ID  → use it directly
 * - If MAILERLITE_GROUP_ID is a name string → search by that name
 * - If MAILERLITE_GROUP_ID is unset         → search by DOWNRANGE_GROUP_NAME ("DownRange")
 * Result is cached per cold start.
 */
async function resolveGroupId() {
  if (_resolvedGroupId) return _resolvedGroupId

  const envVal = process.env.MAILERLITE_GROUP_ID || ''

  // Env var is a numeric ID — use directly
  if (/^\d+$/.test(envVal)) {
    _resolvedGroupId = envVal
    return _resolvedGroupId
  }

  // Env var is a name (or unset) — look up by name
  const searchName = envVal || DOWNRANGE_GROUP_NAME
  ...
  if (!match) {
    throw new Error(`MailerLite group "${searchName}" not found. Groups: ${groups.map(g => g.name).join(', ')}`)
  }

  _resolvedGroupId = match.id
  console.log(`[mailerLite] Resolved group "${match.name}" → id: ${_resolvedGroupId}`)
  return _resolvedGroupId
}
