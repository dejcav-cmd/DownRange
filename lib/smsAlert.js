...
function getSMSConfig() {
  return {
    ...
    criticalJobs: (process.env.SMS_CRITICAL_JOBS ?? 'news,sanity,cron-health')
...
export async function sendSMSAlert(message, options = {}) {
  ...
  if (missing.length) {
    const reason = `Missing env: TWILIO_${missing.map(k => ({ sid:'ACCOUNT_SID', token:'AUTH_TOKEN', from:'FROM_NUMBER', to:'' })[k] || k.toUpperCase()).join(', TWILIO_')}`
 ...
 * Automatically prepends "🚨 DownRange — " to the message.
 */
export async function withSMSAlert(jobId, fn, options = {}) {
  ...
  try {
    ...
    const smsBody = `🚨 DownRange — ${jobId} FAILED\n${msg.slice(0, 130)}\n${new Date().toUTCString().slice(0, 25)}`
...
export function getSMSConfigStatus() {
  ...
  return {
    ...
    enabled:    cfg.enabled,
    from:       cfg.from    ? cfg.from.slice(0, 6) + '…'    : null,
    to:         cfg.to      ? cfg.to.slice(0, 6) + '…'      : null,
    sidSet:     !!cfg.sid,
    tokenSet:   !!cfg.token,
