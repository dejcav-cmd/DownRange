...
async function extractWithClaude(title, pressReleaseText, sourceUrl, manufacturer) {
  const prompt = `You are a DownRange editor — a firearms journalist writing for serious gun owners. Extract product data and write an ORIGINAL DownRange article. Do NOT reproduce manufacturer marketing copy. Rewrite everything in DownRange's voice.
...
Return ONLY a valid JSON object with exactly these fields (no markdown, no preamble):
{
  ...
  "msrp": 0,
  "summary": "3-4 sentence ORIGINAL DownRange summary. Direct, specific, written for carriers and competitors. No marketing fluff. Original analysis only.",
  "specs": [
...
// ── Redis cache layer: avoids Sanity query + Claude call for already-seen URLs ─
// Maintains a Redis set of processed sourceUrl hashes (TTL: 30 days).
// On a hit, returns true immediately — skips both Sanity fetch and AI extraction.
let _relRedis = null
function getRelRedis() {
  if (_relRedis) return _relRedis
  if (!process.env.UPSTASH_REDIS_REST_URL || !process.env.UPSTASH_REDIS_REST_TOKEN) return null
  _relRedis = new UpstashRedis({ url: process.env.UPSTASH_REDIS_REST_URL, token: process.env.UPSTASH_REDIS_REST_TOKEN })
  return _relRedis
}

async function isDuplicate(sourceUrl) {
  // ── Fast path: Redis hash check ──────────────────────────────────────────
  const redis = getRelRedis()
  if (redis) {
    try {
      const key = 'dr:releases:seen'
      const urlHash = sourceUrl.slice(-48) // last 48 chars is unique enough for PRN URLs
      const seen = await redis.sismember(key, urlHash)
      if (seen) return true
    } catch { /* non-critical — fall through to Sanity check */ }
  }

  // ── Slow path: Sanity check (only for new URLs) ──────────────────────────
  try {
...
// Call after a release is successfully saved — adds URL to Redis seen-set
async function markSeen(sourceUrl) {
  const redis = getRelRedis()
  if (!redis) return
  try {
    const key = 'dr:releases:seen'
    const urlHash = sourceUrl.slice(-48)
    await redis.sadd(key, urlHash)
    await redis.expire(key, 30 * 86400) // 30-day TTL
  } catch { /* non-critical */ }
}
...
export async function scrapeReleases({ limit = 15, onlyNew = true } = {}) {
  ...
  for (const manufacturer of MANUFACTURERS) {
    ...
    for (const item of items.slice(0, 5)) {
      ...
      try {
        ...
        await markSeen(item.link)  // Cache URL so future runs skip Sanity query + Claude call
        results.added.push({
          id: doc._id,
          title: extracted.title,
          brand: extracted.brand,
