  ...
  if (variant === 'compact') {
    return (
      <div style={{ padding: '24px', background: '#0D1117', border: '1px solid #C8922A30' }}>
        ...
        {status === 'success' ? (
          <div style={{ color: '#34D399', fontFamily: 'monospace', fontSize: '12px' }}>✓ You're in. First brief hits tomorrow at 7am.</div>
        ) : (
          <form onSubmit={handleSubmit} style={{ display: 'flex', gap: '8px' }}>
            <input type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="your@email.com" required
              style={{ flex: 1, background: '#111318', border: '1px solid #1F2428', color: '#E8E6E1', padding: '8px 10px', fontFamily: 'monospace', fontSize: '12px', minWidth: 0 }} />
            <button type="submit" disabled={status==='loading'}
              style={{ background: '#C8922A', color: '#000', border: 'none', padding: '8px 14px', fontFamily: 'monospace', fontWeight: 700, fontSize: '12px', cursor: 'pointer', whiteSpace: 'nowrap' }}>
              {status === 'loading' ? '...' : 'JOIN →'}
            </button>
          </form>
        )}
        {status === 'error' && <div style={{ color: '#EF4444', fontFamily: 'monospace', fontSize: '11px', marginTop: '6px' }}>Something went wrong. Try again.</div>}
      </div>
    )
  }
    ...
    <section style={{ background: 'linear-gradient(135deg, #0D1117 0%, #111318 50%, #0D0A00 100%)', border: '1px solid #C8922A40', padding: '60px 0', margin: '48px 0' }}>
      <div className="container">
        <div style={{ maxWidth: '680px', margin: '0 auto', textAlign: 'center' }}>
          <div style={{ fontFamily: 'monospace', fontSize: '11px', color: '#C8922A', letterSpacing: '0.2em', marginBottom: '16px' }}>
          ...
          {status === 'success' ? (
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', gap: '0', maxWidth: '480px', margin: '0 auto', flexWrap: 'wrap' }}>
              <input type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="Enter your email address" required
                style={{ flex: 1, minWidth: '240px', background: '#111318', border: '2px solid #1F2428', borderRight: 'none', color: '#E8E6E1', padding: '14px 20px', fontFamily: 'monospace', fontSize: '14px' }} />
              <button type="submit" disabled={status==='loading'}
          ...
          {status === 'error' && <div style={{ color: '#EF4444', fontFamily: 'monospace', fontSize: '12px', marginTop: '12px' }}>Something went wrong. Try again.</div>}
          ...
          <div style={{ display: 'flex', gap: '32px', justifyContent: 'center', marginTop: '28px', flexWrap: 'wrap' }}>
            {['Breaking alerts within minutes','Law changes before they affect you','Ammo price drops in your caliber'].map(t=>(
              <div key={t} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                ...
                <span style={{ fontFamily: 'monospace', fontSize: '11px', color: '#4B5563' }}>{t}</span>
              </div>
            ))}
          </div>
        </div>
