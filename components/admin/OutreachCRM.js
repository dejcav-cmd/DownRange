...
function buildEmailHTML({ subject, preheader, greeting, body, ctaText, ctaUrl, contactName, senderName='DJ Cavalcanti', signature='', accentColor=GOLD }) {
  ...
  const cta = ctaText && ctaUrl ? `<div style="margin:28px 0 8px;"><a href="${ctaUrl}" style="display:block;background:${accentColor};color:#000;padding:14px 36px;text-decoration:none;font-weight:700;font-size:13px;letter-spacing:0.1em;text-transform:uppercase;text-align:center;">${ctaText} &rarr;</a></div>` : ''
  const sig = signature
