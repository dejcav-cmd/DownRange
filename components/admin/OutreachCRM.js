  ...
  async function saveToZohoDrafts(){
    ...
    for(const c of targets.filter(c=>c.email)){
      ...
      const subj=subject
        ...
        .replace(/\{\{businessName\}\}/g,c.name||'')
        .replace(/\{\{channelName\}\}/g,c.name||'')
      const isPlainTpl = true // all templates are plain text
      const greeting_resolved = (greeting||'Hello {{firstName}},').replace(/\{\{firstName\}\}/g,firstName)
      const plainText = isPlainTpl ? greeting_resolved + '\n\n' + personalizedBody : null
      const html = isPlainTpl ? null : buildEmailHTML({subject,preheader,greeting,body:personalizedBody,ctaText,ctaUrl,contactName:firstName,accentColor:accent,signature:sig})
      try{
        const r=await fetch('/api/admin/zoho-draft',{method:'POST',headers:H,body:JSON.stringify({
          to:c.email,
          toName:c.name||'',
          subject:subj,
          ...(isPlainTpl ? {textBody:plainText} : {htmlBody:html}),
        })})
        const d=await r.json()
        if(d.ok) created++
