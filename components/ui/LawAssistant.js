...
export default function LawAssistant() {
  ...
  return (
    <div style={{ background: '#0D1117', border: '1px solid #C8922A40', maxWidth: '720px', margin: '0 auto' }}>
      {/* Header */}
      <div style={{ padding: '20px 24px', borderBottom: '1px solid #1F2428', display: 'flex', alignItems: 'center', gap: '12px' }}>
        ...
        <div>
          ...
          <div style={{ fontFamily: 'monospace', fontSize: '10px', color: '#4B5563' }}>Powered by Claude AI + DownRange state database</div>
        </div>
        <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: '6px' }}>
          <span style={{ width: 6, height: 6, borderRadius: '50%', background: '#34D399', display: 'inline-block' }} />
          <span style={{ fontFamily: 'monospace', fontSize: '10px', color: '#34D399' }}>ONLINE</span>
        </div>
      </div>

      {/* Disclaimer */}
      <div style={{ padding: '10px 24px', background: '#1A0E00', borderBottom: '1px solid #1F2428', fontFamily: 'monospace', fontSize: '10px', color: '#92400E', letterSpacing: '0.05em' }}>
      ...
      <div style={{ minHeight: '280px', maxHeight: '420px', overflowY: 'auto', padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
        {messages.length === 0 && (
          <div style={{ color: '#4B5563', fontFamily: 'monospace', fontSize: '13px', lineHeight: 1.7 }}>
        ...
        {loading && (
          <div style={{ display: 'flex', gap: '10px' }}>
            ...
            <div style={{ background: '#0D1117', border: '1px solid #C8922A30', padding: '12px 16px', fontFamily: 'monospace', fontSize: '12px', color: '#4B5563' }}>
        ...
        <div style={{ padding: '0 24px 16px', display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
          {SUGGESTED.map(q => (
            <button key={q} onClick={() => ask(q)}
              style={{ background: '#111318', border: '1px solid #1F2428', color: '#6B7280', fontFamily: 'monospace', fontSize: '11px', padding: '6px 12px', cursor: 'pointer', textAlign: 'left' }}>
              {q}
            </button>
          ))}
        </div>
      ...
      {/* Input */}
      <div style={{ padding: '16px 24px', borderTop: '1px solid #1F2428', display: 'flex', gap: '10px' }}>
        <input
          value={input}
          onChange={e => setInput(e.target.value)}
          onKeyDown={e => e.key === 'Enter' && ask(input)}
          placeholder="Ask about carry laws, reciprocity, regulations..."
          style={{ flex: 1, background: '#111318', border: '1px solid #1F2428', color: '#E8E6E1', padding: '10px 14px', fontFamily: 'monospace', fontSize: '13px' }}
        />
        <button onClick={() => ask(input)} disabled={loading || !input.trim()}
          style={{ background: '#C8922A', color: '#000', border: 'none', padding: '10px 20px', fontFamily: 'monospace', fontWeight: 700, fontSize: '13px', cursor: 'pointer', opacity: loading || !input.trim() ? 0.5 : 1 }}>
          ASK →
        </button>
      </div>
    </div>
